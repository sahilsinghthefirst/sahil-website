...
test("server renders the Sahil portfolio", async () => {
  ...
  assert.match(html, /href="\/papers\/mig-pino-paper\.pdf"/);
  assert.doesNotMatch(html, /https:\/\/saidlaboratory\.github\.io\/GEML\//);
  assert.doesNotMatch(html, /Project page/);
  assert.match(html, /https:\/\/github\.com\/saidlaboratory\/GEML/);
  assert.match(html, /https:\/\/github\.com\/atharv-santosh\/MineOcclude/);
  assert.match(html, /https:\/\/anonymous\.4open\.science\/r\/cardiac-field-localization-benchmark-E810/);
...
test("keeps the final page free of starter preview infrastructure", async () => {
  ...
  assert.equal((page.match(/className="text-arrow" aria-hidden="true">\{TEXT_ARROWS\.external\}<\/span>/g) ?? []).length, 12);
  ...
  await access(new URL("../public/papers/mineocclude-paper.pdf", import.meta.url));
  await access(new URL("../public/papers/mig-pino-paper.pdf", import.meta.url));
  await access(new URL("../public/assets/mit-logo.png", import.meta.url));
